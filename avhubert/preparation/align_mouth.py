...
def load_args(default_config=None):
    ...
    parser.add_argument(
        "--filename-path", help="list of detected video and its subject ID"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        ...
        assert os.path.isfile(
            video_pathname
        ), "File does not exist. Path input: {}".format(video_pathname)
        assert os.path.isfile(
            landmarks_pathname
        ...
        if not preprocessed_landmarks:
            ...
            frame_gen = read_video(video_pathname)
            frames = [
                cv2.resize(x, (args.crop_width, args.crop_height)) for x in frame_gen
            ]
